...
------   | --------   | ----     |  -----                                                       | -------   | ------
 🟢Template🔴 | [devnet-2m-test](https://template.devnet.io/)   | [Network config](network-configs/devnet-2m-test) / [Inventory](https://bootnode-1.srv.devnet-2m-test.ethpandaops.io/meta/api/v1/inventory.json) / [Validator ranges](https://bootnode-1.srv.devnet-2m-test.ethpandaops.io/meta/api/v1/validator-ranges.json)    | [🔗](ansible/inventories/devnet-2m-test) | [🔗](terraform/devnet-2m-test) | [🔗](kubernetes/devnet-2m-test)
...
## Terraform
From [`./terraform/devnet-2m-test/`](./terraform/devnet-2m-test/)
```shell
terraform init
terraform apply
```
...
ansible-playbook -i inventories/devnet-2m-test/inventory.ini playbook.yaml
```
In order to clean up the deployment
```shell
ansible-playbook -i inventories/devnet-2m-test/inventory.ini cleanup_ethereum.yaml
```
...
1. Open the [main.tf](terraform/devnet-2m-test/main.tf) file located in the terraform/devnet-2m-test/ directory.
...
4. `terraform apply` will create a the machines and the inventory file for you. The inventory file will be located in the [ansible/inventories/devnet-2m-test](ansible/inventories/devnet-2m-test/) directory.
...
6. Adjust the total number of validators in the [ansible/inventories/devnet-2m-test/group_vars/all.yaml](ansible/inventories/devnet-2m-test/group_vars/all.yaml) file (ethereum_genesis_generator_config_files.values.env.NUMBER_OF_VALIDATORS) to match with your total number of validators that you are running.. This will be used by the Ansible playbook to generate the validator keys and deposit data for the network.

## Network Configuration
1. [ansible/inventories/devnet-2m-test/group_vars/all.yaml](ansible/inventories/devnet-2m-test/group_vars/all.yaml) has all the network configuration parameters. Adjust the parameters according to your requirements. Most likely you will not need to adjust these, unless you would like to use a custom setup. The default configuration will work for most networks.
...
ansible-playbook -i inventories/devnet-2m-test/inventory.ini playbook.yaml
```
from the [ansible/](ansible/) directory to deploy the network. This will generate the genesis file, validators and deploy the network according to the configuration parameters specified in the [ansible/inventories/devnet-2m-test/group_vars/all.yaml](ansible/inventories/devnet-2m-test/group_vars/all.yaml) file.  

Don't forget the following gotchas:
- Change the `ethereum_genesis_chain_id` value in [ansible/inventories/devnet-2m-test/group_vars/all.yaml](ansible/inventories/devnet-2m-test/group_vars/all.yaml) to avoid clashing with an existing network
...
2. Run
```shell
ansible-playbook -i inventories/devnet-2m-test/inventory.ini ansible-playbook playbook.yaml -t ethereum_genesis -e ethereum_genesis_cleanup=true
```
...
3. Run `terraform destroy` from the [terraform/devnet-2m-test/](terraform/devnet-2m-test/) directory to delete the nodes. This will remove all the virtual machines and the inventory file. Be careful when running this command, as it will delete all the nodes and the inventory file. You will need to run `terraform apply` again to create the nodes and the inventory file.
...
* Place any custom tooling in the [kubernetes/devnet-2m-test](kubernetes/devnet-2m-test) directory. The tooling will be deployed to the Kubernetes cluster by ArgoCD.
...
* To add a new tool, create a new directory in the [kubernetes/devnet-2m-test](kubernetes/devnet-2m-test) directory. The directory name will be used as the tool name. Place the helm chart in the tool directory. Commit the changes to the repository and ArgoCD will automatically deploy the new tooling to the Kubernetes cluster.
...
* To get the IP addresses of the nodes, run `terraform output` from the [terraform/devnet-2m-test/](terraform/devnet-2m-test/) directory.
* To get the validator ranges run
```shell
curl -s https://bootnode-1.srv.devnet-2m-test.ethpandaops.io/meta/api/v1/validator-ranges.json
```
* To get which validator proposed a specific block run
```shell
ethdo --connection=https://user:password@bn.lighthouse-nethermind-1.srv.devnet-2m-test.ethpandaops.io block info --blockid 100 --json | jq -r .message.proposer_index | ./whose_validator.zsh
```
from the [ansible/](ansible/) directory.
* Getting execution layer client enodes
```shell
curl -s https://config.devnet-2m-test.ethpandaops.io/api/v1/nodes/inventory | jq -r '.ethereum_pairs[] | .execution.enode'
```
* Getting conseus layer client ENRs
```shell
curl -s https://config.devnet-2m-test.ethpandaops.io/api/v1/nodes/inventory | jq -r '.ethereum_pairs[] | .consensus.enr'
```
